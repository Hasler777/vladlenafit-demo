...
;(function () {
  ...
  const uid = queryUid || (parentClarityQueryData && parentClarityQueryData.uid) || cookieUid || "QD3n1Z2UR0583isQS1p969HQvTBKwVPd";
  const sid = (parentClarityQueryData && parentClarityQueryData.sid) || cookieSid || "2eiDQUHR7FLsCZfZcl_oJVZ1BC9bUVmk:1781054823712";
  ...
  function appendSocketStoreToBody() {
    if (false || window.clrtReactivity === true) {
      if (window.clrtIsReactive === false) {
        ...
        script.src = "https://kurs.vladlenafit.ru/chtm/s/metric/socket-bundle.8dU6jrcTp8rM8.js";
  ...
  function getClarityImageUrl(rewrite) {
    var baseUrl = "https://kurs.vladlenafit.ru/chtm/s/metric/clarity.gif"
  ...
  function trackBehaviour() {
    ...
    const browserSessionId = sessionStorage.getItem('clrt-v8-session-id') || "C410VH-Ipqs_HVNzpO-8mTjpdojPVDRy";
    ...
    document.addEventListener("visibilitychange", function sendBehaviourBeacon() {
      if (document.visibilityState === "hidden") {
        navigator.sendBeacon(
          "https://kurs.vladlenafit.ru/chtm/s/metric/behaviourBeacon" + '?' + getBehaviourQueryParams().toString()
        );
      }
    });
    ...
    function appendBehaviourImage() {
      ...
      function appendTrackerToBody() {
        ...
        img.style.width = '1px';
        img.style.height = '1px';
        img.src = "https://kurs.vladlenafit.ru/chtm/s/metric/behaviour.gif" + '?' + getBehaviourQueryParams().toString();
        img.onload = function () {
          img.remove();
          loading = false
        };
